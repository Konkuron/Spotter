...
async def skip(ctx, *, number):

    try:
        number = int(number)
    except:
        await ctx.send("This command requires either no input or a numeric input in order to work.")

    num_format = isinstance(number, int)
...
@client.command(name="sCommands")
async def sCommands(ctx):
